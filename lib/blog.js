...
export function postToRow(post) {
  return {
    ...
    category: post.category || 'Aprendendo com a FortePix',
    tags: post.tags || [],
...
export async function savePosts(posts) {
  ...
  for (const post of posts) {
    ...
    const authorJson = JSON.stringify(row.author || { name: 'Equipe FortePix' });
...
export async function seedIfEmpty() {
  try {
    ...
    const data = loadJsonFallback();
    const filePosts = (data.posts || []).map((p) => ({ ...p, status: 'published' }));
    if (!filePosts.length) return { seeded: false, reason: 'no_file_posts' };
    ...
    const same =
      dbSlugs.size === fileSlugs.size &&
      [...fileSlugs].every((slug) => dbSlugs.has(slug));

    if (same) {
      return { seeded: false, reason: 'already_synced', total: dbSlugs.size };
    }

    // Atualiza o banco com o conteúdo educativo do posts.json (substitui o conjunto antigo)
    return await savePosts(filePosts);
  } catch (err) {
